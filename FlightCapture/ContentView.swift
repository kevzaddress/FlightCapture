...
// Confidence scoring enums and helpers
...
func calculateFlightNumberConfidence(_ text: String) -> ConfidenceLevel {
    ...
    if trimmed.isEmpty { return .low }
    if trimmed.range(of: "^[A-Z0-9]{2,4}$", options: .regularExpression) != nil { return .high }
    if trimmed.range(of: "^[A-Z0-9]{1,6}$", options: .regularExpression) != nil { return .medium }
    return .low
}
...
func calculateAircraftRegConfidence(_ text: String) -> ConfidenceLevel {
    let trimmed = text.trimmingCharacters(in: .whitespacesAndNewlines)
    if trimmed.isEmpty { return .low }
    if trimmed.range(of: "^[A-Z]-[A-Z0-9]{3,4}$", options: .regularExpression) != nil { return .high }
    if trimmed.range(of: "^[A-Z][A-Z0-9]{3,4}$", options: .regularExpression) != nil { return .medium }
    return .low
}
...
func calculateAirportConfidence(_ text: String) -> ConfidenceLevel {
    let trimmed = text.trimmingCharacters(in: .whitespacesAndNewlines)
    if trimmed.isEmpty { return .low }
    if trimmed.range(of: "^[A-Z]{3,4}$", options: .regularExpression) != nil { return .high }
    if trimmed.range(of: "^[A-Z]{2,5}$", options: .regularExpression) != nil { return .medium }
    return .low
}
...
func calculateCrewNameConfidence(_ text: String) -> ConfidenceLevel {
    let trimmed = text.trimmingCharacters(in: .whitespacesAndNewlines)
    if trimmed.isEmpty { return .low }
    if trimmed.hasSuffix("...") { return .low }
    if trimmed.range(of: "^[A-Za-z\\s\\-\\.]+$", options: .regularExpression) != nil && trimmed.count >= 2 { return .high }
    if trimmed.range(of: "^[A-Za-z\\s\\-\\.]+$", options: .regularExpression) != nil { return .medium }
    return .low
}

// Flight data for review
...
struct ContentView: View {
    ...
    func getCrewNamesAndCodes() -> [(role: String, name: String)] {
        var result: [(role: String, name: String)] = []
        result.append(("Commander", extractCrewName(from: ocrCrewCommander, role: "Commander")))
        result.append(("SIC", extractCrewName(from: ocrCrewSIC, role: "SIC")))
        result.append(("Relief", extractCrewName(from: ocrCrewRelief, role: "Relief")))
        result.append(("Relief2", extractCrewName(from: ocrCrewRelief2, role: "Relief2")))
        result.append(("ISM", extractCrewName(from: ocrCrewCustom2ISM, role: "ISM")))
        result.append(("SP", extractCrewName(from: ocrCrewCustom4SP, role: "SP")))
        result.append(("FP", extractCrewName(from: ocrCrewCustom1FP, role: "FP")))
        result.append(("FlightAttendant", extractCrewName(from: ocrCrewFlightAttendant, role: "FlightAttendant")))
        result.append(("FlightAttendant2", extractCrewName(from: ocrCrewFlightAttendant2, role: "FlightAttendant2")))
        result.append(("FlightAttendant3", extractCrewName(from: ocrCrewFlightAttendant3, role: "FlightAttendant3")))
        result.append(("FlightAttendant4", extractCrewName(from: ocrCrewFlightAttendant4, role: "FlightAttendant4")))
        return result
    }
    ...
    var body: some View {
        NavigationView {
            MainContentView(
                ...
                onTime: onTime,
                inTime: inTime,
                showLogTenAlert: $showLogTenAlert,
                showCrewReviewSheet: $showCrewReviewSheet,
                showReviewAllDataSheet: $showReviewAllDataSheet,
                crewNamesNeedingReview: $crewNamesNeedingReview,
                onPhotoSelection: handlePhotoSelection,
                onCheckCrewNames: checkCrewNamesForReview,
                onExportToLogTen: { exportToLogTen(jsonString: generateLogTenJSON()) },
                onIsLogTenInstalled: isLogTenProInstalled
            )
        }
        .sheet(isPresented: $showCrewReviewSheet) {
            CrewReviewModal(isPresented: $showCrewReviewSheet, crewNamesNeedingReview: $crewNamesNeedingReview)
        }
        .sheet(isPresented: $showReviewAllDataSheet) {
            ReviewAllDataModal(isPresented: $showReviewAllDataSheet, flightData: getFlightDataForReview())
        }
    }
    
    // Main Content View to break down complexity
    struct MainContentView: View {
        ...
        let onTime: String?
        let inTime: String?
        @Binding var showLogTenAlert: Bool
        @Binding var showCrewReviewSheet: Bool
        @Binding var showReviewAllDataSheet: Bool
        @Binding var crewNamesNeedingReview: [CrewNameReview]
        let onPhotoSelection: ([PhotosPickerItem]) -> Void
        let onCheckCrewNames: () -> Bool
        let onExportToLogTen: () -> Void
        let onIsLogTenInstalled: () -> Bool
                ...
                VStack(spacing: 24) {
                    ...
                    if flightNumber != nil && aircraftReg != nil && departureAirport != nil && arrivalAirport != nil {
                        ExportSection(
                            showLogTenAlert: $showLogTenAlert,
                            showCrewReviewSheet: $showCrewReviewSheet,
                            onCheckCrewNames: onCheckCrewNames,
                            onExportToLogTen: onExportToLogTen,
                            onIsLogTenInstalled: onIsLogTenInstalled
                        )
                    }
                }
            }
        ...
        var body: some View {
            VStack(spacing: 16) {
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        HStack {
                            Image(systemName: "circle.fill")
                                    ...
                                    .font(.caption)
                            Text("Green: High confidence - data looks accurate")
                                .font(.subheadline)
                                .foregroundColor(.secondary)
                        }
                        ...
                        HStack {
                            Image(systemName: "circle.fill")
                                .foregroundColor(.yellow)
                                    .font(.caption)
                            Text("Yellow: Medium confidence - may need review")
                                .font(.subheadline)
                                .foregroundColor(.secondary)
                        }
                        ...
                        HStack {
                            Image(systemName: "circle.fill")
                                .foregroundColor(.red)
                                .font(.caption)
                            Text("Red: Low confidence - review recommended")
                                .font(.subheadline)
                                .foregroundColor(.secondary)
                        }
                    }
                ...
                Button(action: {
                    ...
                    if onCheckCrewNames() {
                        print("[DEBUG] Names need review, showing modal.")
                        DispatchQueue.main.async {
                            showCrewReviewSheet = true
                        }
                    } else if onIsLogTenInstalled() {
                        onExportToLogTen()
                    } else {
                        showLogTenAlert = true
                    }
    ...
    func getFlightDataForReview() -> [FlightDataForReview] {
        print("[DEBUG] Preparing flight data for review")
        return [
            FlightDataForReview(
                field: "Flight",
                originalValue: "\(flightNumber ?? "Not available") - \(departureAirport ?? "Not available") - \(arrivalAirport ?? "Not available")",
                editedValue: "\(flightNumber ?? "Not available") - \(departureAirport ?? "Not available") - \(arrivalAirport ?? "Not available")",
                confidence: .low,
                editedDate: nil
            ),
            FlightDataForReview(
                field: "OutTime",
                originalValue: outTime ?? "",
                editedValue: editedOutTime,
                confidence: .low,
                editedDate: nil
            ),
            FlightDataForReview(
                field: "OffTime",
                originalValue: offTime ?? "",
                editedValue: editedOffTime,
                confidence: .low,
                editedDate: nil
            ),
            FlightDataForReview(
                field: "OnTime",
                originalValue: onTime ?? "",
                editedValue: editedOnTime,
                confidence: .low,
                editedDate: nil
            ),
            FlightDataForReview(
                field: "InTime",
                originalValue: inTime ?? "",
                editedValue: editedInTime,
                confidence: .low,
                editedDate: nil
            )
        ]
    ...
    func applyReviewedData(_ reviewedData: [FlightDataForReview]) {
        ...
        editedFlightNumber = reviewedData[0].editedValue.components(separatedBy: " - ")[0]
        editedAircraftReg = reviewedData[0].editedValue.components(separatedBy: " - ")[1]
        editedDeparture = reviewedData[0].editedValue.components(separatedBy: " - ")[2]
        editedArrival = reviewedData[0].editedValue.components(separatedBy: " - ")[3]
        editedDate = reviewedData[0].editedDate
        editedOutTime = reviewedData[1].editedValue
        editedOffTime = reviewedData[2].editedValue
        editedOnTime = reviewedData[3].editedValue
        editedInTime = reviewedData[4].editedValue
        
        print("[DEBUG] Applied reviewed data: Flight=\(reviewedData[0].editedValue), Aircraft=\(reviewedData[0].editedValue.components(separatedBy: " - ")[1]), From=\(reviewedData[0].editedValue.components(separatedBy: " - ")[2]), To=\(reviewedData[0].editedValue.components(separatedBy: " - ")[3])")
    }
        ...
        if let cropped = cropImage(uiImage, to: dashboardCrewNamesROI) {
            ocrText(from: cropped, label: "DashboardCrewNames") { crewText in
                DispatchQueue.main.async {
                    ...
                    let crewNames = parseDashboardCrewText(crewText)
                    self.parsedCrewList = []
...
struct CrewReviewModal: View {
    ...
    var body: some View {
        NavigationView {
            List {
                ForEach($crewNamesNeedingReview) { $review in
                    VStack(alignment: .leading, spacing: 8) {
                        Text(review.role)
                            .font(.headline)
                        Text("Original: \(review.original)")
                            .font(.caption)
                            .foregroundColor(.secondary)
                        TextField("Corrected name", text: $review.corrected)
...
struct ReviewAllDataModal: View {
    ...
    var body: some View {
        NavigationView {
            List {
                ForEach(flightData, id: \.field) { data in
                    VStack(alignment: .leading, spacing: 8) {
                        HStack {
                            Text(data.field)
                                .font(.headline)
                            Spacer()
                            Circle()
                                .fill(data.confidence.color)
                                .frame(width: 12, height: 12)
                        }
                        Text("Original: \(data.originalValue)")
                            .font(.caption)
                            .foregroundColor(.secondary)
                        Text("Current: \(data.editedValue)")
                            .font(.body)
                    }
                    .padding(.vertical, 4)
                }
...
#Preview {
    ContentView(incomingImageURL: .constant(nil as URL?))
}
